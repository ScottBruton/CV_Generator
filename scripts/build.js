...
class CVBuilder {
  ...
  loadContent() {
    return {
      ...
      versions: loadJson('app', 'versions.json'),
      cover: loadJson('cover', 'default.json'),
...
${timelineHtml}
  ...
  /** Build dropdown option buttons for the dashboard header. */
  buildDashboardOptions(items, view) {
    return (items || [])
      .map((item) => {
        const versionId = escapeHtml(item.id || 'default');
        const label = escapeHtml(item.label || item.id || 'Default');
        return `<li role="none"><button type="button" class="app-dropdown__option" role="menuitem" data-view="${view}" data-version="${versionId}">${label}</button></li>`;
      })
      .join('\n');
  }
  ...
  buildCover(content) {
    ...
    return renderComponent('cover-page', {
      ...
      companyLine: cover.company ? escapeHtml(cover.company) : '',
  ...
  build() {
    ...
    const html = '<!-- Generated by scripts/build.js — edit content/*.json and components, then save (watch rebuilds) or run npm run build. Do not edit this file directly. -->\n' + render(pageTemplate, {
      pageTitle: `${content.profile.firstName} ${content.profile.lastName} — CV Generator`,
      dashboard: this.buildDashboard(content),
      cover: this.buildCover(content),
